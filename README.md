...
The four preconditioning program slots of the vehicle are writable. Each slot has a **time** entity for the time the vehicle should be ready, a **text** entity for the days and a **switch** entity, plus a button and a switch that handle clearing them. Schedules can also be written from automations with the `stellantis_vehicles.set_preconditioning_program` service.

Details, examples and what was measured on a real car: [docs/preconditioning.md](./docs/preconditioning.md).
