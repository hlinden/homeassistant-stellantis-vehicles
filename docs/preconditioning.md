...
- A command sent while the vehicle is asleep can time out and is then lost. A write at 05:33 went Accepted, Wakeup Vehicle, Timeout, and nothing had changed. Nothing retries it. The automatic clearing does retry, because the slot stays spent until a clear succeeds.
